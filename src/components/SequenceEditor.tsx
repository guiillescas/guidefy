...
export function SequenceEditor() {
  ...
  const editingItem = selectedSong?.sequence.find(item => item.id === editingNoteId);

  if (!selectedSong) {
    return <div className="flex-1 flex items-center justify-center text-gray-500">Select a song to start editing</div>;
  }

  const sequenceSummary = selectedSong?.sequence
    .map(item => {
      const abbr = getAbbreviation(item.element);
      return item.type === 'base' && item.occurrence && item.occurrence > 1 ? `${abbr}${item.occurrence}` : abbr;
    })
    .join(', ');
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="max-w-2xl">
          <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            <SortableContext items={selectedSong?.sequence || []} strategy={verticalListSortingStrategy}>
          ...
          {selectedSong?.sequence.length === 0 && (
            <div className="text-gray-500 text-center py-8">Add elements to create your sequence</div>
          )}
        </div>
      </div>

      {/* ElementSelector para desktop */}
      <div className="hidden md:block">{elementSelectorContent}</div>
